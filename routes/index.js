...
/*  Create an Express application with the following endpoints:
    ...
    /* router.post('/dishes', async (req, res) => {
      ...
      if (existingDishes == null) {
        ...
        await dishesCollection.set("dishes", newDishes);
      } else {
        // If the "dishes" object exists, add the new dish to it
        const dishCount = Object.keys(existingDishes.props).length + 1;
        const newDishKey = `dish${dishCount}`;
        existingDishes.props[newDishKey] = {
          name: dishName,
          country: country
        };
